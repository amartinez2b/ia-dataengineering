# Ejercicio 03 - Instalar MySQL MCP en Codex y generar un modelo de datos

## Objetivo

Instalar el MCP de MySQL en Codex, conectarse a la base `fake`, leer las tablas reales y generar un modelo de datos inicial con ayuda del agente.
...
## Parte A - Instalar el MCP de MySQL en Codex

Usa el comando:

```bash
codex mcp add mcp_server_mysql \
  --env MYSQL_HOST="www.bigdataybi.com" \
  --env MYSQL_PORT="3306" \
  --env MYSQL_USER="curso" \
  --env MYSQL_PASS="PASSWORD_PROPORCIONADO_POR_EL_INSTRUCTOR" \
  --env MYSQL_DB="fake" \
...
- registra un servidor MCP llamado `mcp_server_mysql`
- conecta el agente a la base `fake`
- mantiene deshabilitadas las operaciones de escritura

## Parte B - Verificar la configuración en Codex

Valida el servidor con:

```bash
codex mcp list
codex mcp get mcp_server_mysql
```
...
Ahora pide a Codex una lectura más estructurada del esquema.
...
1. evidencia de instalación del MCP de MySQL en Codex
...
- instalar el MCP de MySQL en Codex
